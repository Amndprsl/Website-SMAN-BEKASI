<h1>Amanda Pricilia Noorhaliza Shaleh. NIM: 1121093000106</h1>
...
<script>
    ...
    var smanbekasi = new L.LayerGroup();
    ...
    var groupedOverlays = {
        ...
        "Peta Khusus":{'SMAN DI BEKASI' :smanbekasi},
    };
    ...
    $.getJSON("<?=base_url()?>assets/smanbekasi.geojson",function(data){ 
        var ratIcon = L.icon({ 
            iconUrl: '<?=base_url()?>assets/Marker-1.png', 
            iconSize: [12,10] 
        }); 
        L.geoJson(data,{ 
            pointToLayer: function(feature,latlng){ 
                var marker = L.marker(latlng,{icon: ratIcon}); 
                marker.bindPopup(feature.properties.nama_tempat + "<br>" + "<span style='color: blue;'>Alamat: " 
                + feature.properties.nama_jalan + "</span>");
                return marker; 
            } 
        }).addTo(smanbekasi); 
    });
